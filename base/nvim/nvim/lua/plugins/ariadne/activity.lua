-- Recent-activity overview, backed by `ariadne-graph --since`
...
local function run_notes_graph(window)
  return utils.run_ariadne_tool("ariadne-graph", { "--since", window }, { "active_clusters" })
end
...
function M.show(window)
  ...
  -- Clusters first, in the order ariadne-graph ranked them: most-touched first, which
...
function M.setup()
  vim.api.nvim_create_user_command("AriadneActive", function(opts)
    M.show(opts.args)
