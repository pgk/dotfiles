...
local function neglected_pool()
  if vim.fn.executable("ariadne-graph") == 0 then
    vim.notify("ariadne-graph not found on PATH; skipping the neglected section", vim.log.levels.WARN)
    return nil
  end
  local decoded = utils.run_ariadne_tool(
    "ariadne-graph",
...
function M.setup()
  vim.api.nvim_create_user_command("AriadneDaily", function(opts)
    local offset = tonumber(opts.args) or 0
    M.open(offset)
  end, { nargs = "?", desc = "Open daily note with template" })

  vim.api.nvim_create_user_command("AriadneDailyReview", function()
    M.add_review()
