...
"""Tests for notes-graph --since, the recent-activity report.
...
class ParseDurationTests(unittest.TestCase):
    def test_understands_hours_days_and_weeks(self):
        self.assertEqual(notes_graph.parse_duration("36h"), 36 * 3600)
        self.assertEqual(notes_graph.parse_duration("7d"), 7 * DAY)
        self.assertEqual(notes_graph.parse_duration("2w"), 14 * DAY)

    def test_accepts_a_fraction_and_ignores_case_and_spacing(self):
        self.assertEqual(notes_graph.parse_duration("  1.5D "), 1.5 * DAY)

    def test_rejects_a_bare_number(self):
        with self.assertRaises(ValueError) as ctx:
            notes_graph.parse_duration("7")
        self.assertIn("7d", str(ctx.exception))

    def test_rejects_an_unknown_unit_and_nonsense(self):
        for bad in ("7x", "abc", "", "d"):
            with self.assertRaises(ValueError):
                notes_graph.parse_duration(bad)
    ...
    def test_the_error_strips_control_characters(self):
        with self.assertRaises(ValueError) as ctx:
            notes_graph.parse_duration("ev\x1b[31mil")
        self.assertNotIn("\x1b", str(ctx.exception))


class DescribeAgeTests(unittest.TestCase):
    def test_scales_from_minutes_to_days(self):
        self.assertEqual(notes_graph.describe_age(0), "0m")
        self.assertEqual(notes_graph.describe_age(90 * 60), "1h")
        self.assertEqual(notes_graph.describe_age(3 * DAY + 3600), "3d")

    def test_a_negative_age_from_a_future_mtime_does_not_go_negative(self):
        self.assertEqual(notes_graph.describe_age(-60), "0m")
...
class TouchedSinceTests(unittest.TestCase):
    def test_keeps_only_notes_modified_after_the_cutoff(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            files = list(notes_common.iter_markdown_files(tmp, []))
            recent = notes_graph.touched_since(files, time.time() - 7 * DAY)
    ...
    def test_an_unreadable_note_warns_rather_than_failing_the_run(self):
        ...
        with contextlib.redirect_stderr(stderr):
            recent = notes_graph.touched_since(["/no/such/note.md"], 0)
...
class ActivityTests(unittest.TestCase):
    def report(self, files, ages, window_days=7):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            labels = notes_cluster.louvain(notes_cluster.adjacency(graph))
            now = time.time()
            recent = notes_graph.touched_since(paths, now - window_days * DAY)
            return notes_graph.activity(graph, labels, recent, tmp, now)
...
class ActivityReportTests(unittest.TestCase):
    ...
    def render(self, clusters, orphaned, touched=1):
        return notes_graph.format_activity(clusters, orphaned, "7d", touched, 9, "/v", self.SHAPE)
