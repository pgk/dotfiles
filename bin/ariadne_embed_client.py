"""OpenAI-compatible embeddings client for ariadne-similar.
...
def safe_url(url):
    ...
    return ariadne_common.printable(urllib.parse.urlunsplit((parts.scheme, netloc, parts.path, "", "")))
...
def validate_endpoint(endpoint, allow_remote):
    ...
    if not is_loopback(host) and not allow_remote:
        raise ValueError(
            f"endpoint host '{ariadne_common.printable(host)}' is not loopback — note text would "
...
def _fetch(url, where, payload, timeout):
    ...
    try:
        ...
        detail = ariadne_common.printable(exc.read(MAX_ERROR_BYTES).decode("utf-8", "replace"))[:200].strip()
        raise EmbedUnavailable(f"{where}: HTTP {exc.code} {detail}") from exc
    except urllib.error.URLError as exc:
        raise EmbedUnavailable(f"{where}: {ariadne_common.printable(exc.reason)}") from exc
    except (OSError, ValueError) as exc:
        raise EmbedUnavailable(f"{where}: {ariadne_common.printable(exc)}") from exc
...
def embed_batch(texts, endpoint, model, timeout=TIMEOUT):
    ...
    for item in _ordered(data, len(texts), where):
        ...
        if not isinstance(values, list) or not values or len(values) > ariadne_embed_cache.MAX_DIMS:
