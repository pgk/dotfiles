...
"""Tests for ariadne-similar's argument handling and its CLI surface.
...
class ArgumentTests(unittest.TestCase):
    ...
    def split(self, argv):
        return ariadne_similar.split_positional(ariadne_similar.parse_args(argv))
    ...
    def check(self, argv):
        ariadne_similar.check_args(ariadne_similar.parse_args(argv))
...
class FullQueryTests(unittest.TestCase):
    ...
    def test_index_then_query_returns_ranked_unlinked_notes(self):
        ...
        with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
            self.assertEqual(ariadne_similar.run_index(args, self.vault, self.notes, fake_embedder()), 0)
            # Second pass must reuse the cache rather than re-embed.
            query = ariadne_similar.run_query(
                args, self.vault, "target", self.notes, self.name_index, fake_embedder(calls=calls)
            )
        self.assertEqual((query, calls), (0, []))

        cdir = ariadne_embed_cache.cache_dir(self.vault)
        cached, dims = ariadne_embed_cache.load_cache(cdir, args.model)
        self.assertEqual(dims, 16)
        target = ariadne_similar.resolve_target("target", self.notes, self.name_index)
        names = [r["name"] for r in ariadne_similar.find_similar(target, self.notes, cached, self.name_index, 10, False)]
    ...
    def test_the_cache_lands_outside_the_vault_and_inside_the_cache_dir(self):
        args = ariadne_similar.parse_args(["target", self.vault, "--json"])
        with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
            ariadne_similar.run_index(args, self.vault, self.notes, fake_embedder())
    ...
    def test_editing_a_note_re_embeds_only_that_note(self):
        ...
        calls = []
        with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
            ariadne_similar.run_query(args, self.vault, "target", notes, name_index, fake_embedder(calls=calls))
    ...
    def test_a_failure_partway_through_a_first_index_keeps_what_was_embedded(self):
        ...
        write_vault(big.name, {f"n{i}.md": f"note number {i}" for i in range(ariadne_embed_cache.BATCH_SIZE + 8)})
        notes = ariadne_similar.scan_vault(big.name, [])
        args = ariadne_similar.parse_args(["--index", big.name])

        embedded = []

        def flaky(texts):
            if embedded:
                raise ariadne_embed_cache.EmbedUnavailable("server went away")
            embedded.extend(texts)
            return fake_embedder()(texts)

        with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
            with self.assertRaises(ariadne_embed_cache.EmbedUnavailable):
                ariadne_similar.run_index(args, big.name, notes, flaky)

        # dims is still 0 when the first index fails, so this is exactly the case
        # where the partial save used to be silently skipped.
        cached, dims = ariadne_embed_cache.load_cache(ariadne_embed_cache.cache_dir(big.name), args.model)
        self.assertEqual(len(embedded), ariadne_embed_cache.BATCH_SIZE)
        self.assertEqual(len(cached), ariadne_embed_cache.BATCH_SIZE)
        self.assertEqual(dims, 16)

    def test_rebuild_re_embeds_everything(self):
        index_args = ariadne_similar.parse_args(["--index", self.vault])
        with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
            ariadne_similar.run_index(index_args, self.vault, self.notes, fake_embedder())

        rebuild_args = ariadne_similar.parse_args(["--index", "--rebuild", self.vault])
        calls = []
        with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
            ariadne_similar.run_index(rebuild_args, self.vault, self.notes, fake_embedder(calls=calls))
