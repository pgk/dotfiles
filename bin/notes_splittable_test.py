...
"""Tests for notes-graph --splittable candidates: long, multi-section notes that
...
class NoteStatsTests(unittest.TestCase):
    def test_word_count_excludes_frontmatter(self):
        ...
        stats = notes_splittable.note_stats("/v/n.md", raw, {}, set())
    ...
    def test_out_degree_counts_distinct_resolved_targets(self):
        ...
        stats = notes_splittable.note_stats("/v/n.md", raw, name_index, files)
        self.assertEqual(stats["out_degree"], 2)

    def test_out_degree_excludes_self_link(self):
        name_index = {"n": "/v/n.md"}
        stats = notes_splittable.note_stats("/v/n.md", "[[n]]", name_index, {"/v/n.md"})
        self.assertEqual(stats["out_degree"], 0)

    def test_out_degree_excludes_unresolved_links(self):
        stats = notes_splittable.note_stats("/v/n.md", "[[nowhere]]", {}, {"/v/n.md"})
        self.assertEqual(stats["out_degree"], 0)
    ...
    def test_out_degree_excludes_a_resolved_target_outside_the_scanned_set(self):
        ...
        stats = notes_splittable.note_stats("/v/n.md", "[[elsewhere]]", name_index, {"/v/n.md"})
        self.assertEqual(stats["out_degree"], 0)
...
class SelectTests(unittest.TestCase):
    def run_select(self, root, min_headers=3, min_words=1200, max_out_degree=8):
        ...
        return notes_splittable.select(files, name_index, min_headers, min_words, max_out_degree, root)
...
class DevVaultFixtureTests(unittest.TestCase):
    """Pins the fixture contract documented in obsidian/CLAUDE.md and PLAN-0006:
    ...
    def test_candidate_is_flagged_and_the_index_is_vetoed_at_the_exact_boundary(self):
        files = list(notes_common.iter_markdown_files(str(DEV_VAULT), []))
        name_index = notes_common.build_name_index(files)
        candidates = notes_splittable.select(files, name_index, 3, 1200, 8, str(DEV_VAULT))
        ...
        with tempfile.TemporaryDirectory() as root:
            ...
            try:
                ...
                with contextlib.redirect_stderr(stderr):
                    candidates = notes_splittable.select(files, name_index, 3, 1200, 8, root)
            finally:
