...
def select(files, name_index, min_headers, min_words, max_out_degree, vault):
    ...
    for path in files:
        try:
            ...
            print(f"warning: skipping unreadable note {ariadne_common.printable(path)}: "
                  f"{ariadne_common.printable(exc)}", file=sys.stderr)
            continue
...
def format_lines(entries):
    """Picker/report rows for a 'Splittable' section — for ariadne-graph to splice in."""
