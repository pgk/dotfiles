...
"""Tests for bin/ariadne-embed-setup, against a fake `ollama` on $PATH.
...
SCRIPT_PATH = Path(__file__).with_name("ariadne-embed-setup")

_loader = importlib.machinery.SourceFileLoader("ariadne_embed_setup", str(SCRIPT_PATH))
_spec = importlib.util.spec_from_loader("ariadne_embed_setup", _loader)
ariadne_embed_setup = importlib.util.module_from_spec(_spec)
_loader.exec_module(ariadne_embed_setup)
...
def run_main(argv):
    ...
    with redirect_stdout(out), redirect_stderr(err):
        code = ariadne_embed_setup.main(argv)
...
class ModelPresentTests(unittest.TestCase):
    ...
    def test_a_different_tag_of_the_same_model_is_not_a_match(self):
        ...
        # present doesn't satisfy that, and ariadne-similar would fail against :latest
