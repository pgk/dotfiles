"""Shared harness for the notes-similar test suites.

`notes-similar` is a hyphenated script, so it cannot be imported normally; loading
it, and the deterministic stand-in embedder, live here so the suites that split off
notes-similar_test.py do not each carry their own copy.
"""
...
_loader = importlib.machinery.SourceFileLoader("notes_similar", str(SCRIPT_PATH))
_spec = importlib.util.spec_from_loader("notes_similar", _loader)
notes_similar = importlib.util.module_from_spec(_spec)
_loader.exec_module(notes_similar)
