...
"""Tests for cluster-level health: coverage, and the clusters ariadne-graph reports.

Split out of ariadne_cluster_test.py and ariadne-graph_test.py, which the feature
...
class ClusterEntryPointTests(unittest.TestCase):
    ...
    def entries(self, graph, labels=None):
        return ariadne_cluster.cluster_entry_points(graph, labels or ariadne_cluster.louvain(graph))
    ...
    def test_empty_vault(self):
        self.assertEqual(ariadne_cluster.cluster_entry_points({}, {}), [])
...
class HublessClusterTests(unittest.TestCase):
    def build(self, files, hub_reach=0.5):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            return ariadne_graph.hubless_clusters(neighbors, labels, hub_reach, tmp)
...
class HublessReportTests(unittest.TestCase):
    ...
    def test_report_states_the_cluster_size_and_the_best_reach(self):
        out = "\n".join(ariadne_graph.format_hubless([self.ENTRY]))
    ...
    def test_control_characters_in_a_note_name_are_stripped(self):
        out = "\n".join(ariadne_graph.format_hubless([dict(self.ENTRY, name="ev\x1b[31mil", rel="a\x07.md")]))
        for bad in ("\x1b", "\x07"):
    ...
    def test_text_report_carries_the_graph_shape(self):
        ...
        out = ariadne_graph.format_text([], [], 3, 8, "/v", [self.ENTRY], self.SHAPE)
    ...
    def test_the_shape_line_is_omitted_when_there_is_no_shape(self):
        out = ariadne_graph.format_text([], [], 3, 8, "/v", [self.ENTRY])
        self.assertNotIn("graph:", out)

    def test_all_clear_message_mentions_hubs_too(self):
        out = ariadne_graph.format_text([], [], 3, 5, "/v")
