...
"""Tests for bin/ariadne-deadlinks, run against synthetic notes only (never ~/notes)."""
...
SCRIPT_PATH = Path(__file__).with_name("ariadne-deadlinks")

loader = importlib.machinery.SourceFileLoader("ariadne_deadlinks", str(SCRIPT_PATH))
spec = importlib.util.spec_from_loader("ariadne_deadlinks", loader)
ariadne_deadlinks = importlib.util.module_from_spec(spec)
loader.exec_module(ariadne_deadlinks)
...
class CollectDeadLinksTests(unittest.TestCase):
    def collect(self, files, excludes=None):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            dead_by_file = ariadne_deadlinks.collect_dead_links(paths, index)
            report = ariadne_deadlinks.build_report(dead_by_file, index)
            return report
...
class FormatTests(unittest.TestCase):
    def test_text_reports_no_dead_links(self):
        text = ariadne_deadlinks.format_text([], 3, "/vault")
    ...
    def test_text_includes_candidates_and_no_matches_label(self):
        ...
        text = ariadne_deadlinks.format_text(report, 2, "/vault")
    ...
    def test_text_strips_control_characters_from_link_and_candidates(self):
        ...
        text = ariadne_deadlinks.format_text(report, 1, "/vault")
    ...
    def test_json_shape_and_counts(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            dead_by_file = ariadne_deadlinks.collect_dead_links(paths, index)
            report = ariadne_deadlinks.build_report(dead_by_file, index)
            payload = json.loads(ariadne_deadlinks.format_json(report, len(paths), tmp))
