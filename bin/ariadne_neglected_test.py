...
"""Tests for ariadne-graph --neglected, the resurface-by-neglect report.
...
def select(root, cutoff_days, min_links=3, limit=None):
    """The selection ariadne-graph performs, against the vault at `root`."""
    now = time.time()
    files = list(ariadne_common.iter_markdown_files(root, []))
    graph = ariadne_cluster.build_graph(files, ariadne_common.build_name_index(files))
    stale = ariadne_neglected.untouched_before(files, now - cutoff_days * DAY)
    return ariadne_neglected.select(graph, stale, min_links, limit, root, now)
...
class SelectionTest(unittest.TestCase):
    ...
    def test_the_cutoff_boundary_is_inclusive(self):
        ...
        with tempfile.TemporaryDirectory() as root:
            ...
            self.assertIn(path, ariadne_neglected.untouched_before([path], exact))
            self.assertNotIn(path, ariadne_neglected.untouched_before([path], exact - 1))
...
class AgeDescriptionTest(unittest.TestCase):
    def test_spans_read_in_the_largest_useful_unit(self):
        describe = ariadne_neglected.describe_span
