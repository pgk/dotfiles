...
"""Tests for bin/ariadne_cluster.py, run against synthetic notes only (never ~/notes)."""
...
class AdjacencyFromLinksTests(unittest.TestCase):
    """The in-memory path, for callers like ariadne-similar that already read the notes."""
    ...
    def test_agrees_with_build_graph(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            entries = [
                {"path": p, "links": ariadne_common.extract_links(Path(p).read_text())}
                for p in paths
            ]
            self.assertEqual(ariadne_cluster.adjacency_from_links(entries, index), from_files)
    ...
    def test_links_outside_the_scanned_set_are_not_edges(self):
        ...
        self.assertEqual(ariadne_cluster.adjacency_from_links(entries, index), {paths[0]: set()})

    def test_self_links_are_not_edges(self):
        entries = [{"path": "/v/a.md", "links": ["a"]}]
        index = {"a": "/v/a.md"}
        self.assertEqual(ariadne_cluster.adjacency_from_links(entries, index), {"/v/a.md": set()})
...
class ComponentsTests(unittest.TestCase):
    def test_splits_disconnected_groups(self):
        ...
        self.assertEqual(ariadne_cluster.components(graph), [["c", "d", "e"], ["a", "b"]])

    def test_isolated_nodes_are_their_own_component(self):
        graph = undirected([("a", "b")], isolated=["z"])
        self.assertEqual(ariadne_cluster.components(graph), [["a", "b"], ["z"]])

    def test_orders_largest_first_then_alphabetically(self):
        graph = undirected([("b", "c"), ("a", "z")])
        self.assertEqual(ariadne_cluster.components(graph), [["a", "z"], ["b", "c"]])

    def test_empty_graph_has_no_components(self):
        self.assertEqual(ariadne_cluster.components({}), [])
...
class LouvainTests(unittest.TestCase):
    ...
    def test_recovers_three_planted_cliques(self):
        ...
        edges += [("d", "e"), ("h", "i")]
        labels = ariadne_cluster.louvain(undirected(edges))
        self.assertEqual(
            grouping(labels),
    ...
    def test_a_clique_is_one_community(self):
        ...
        self.assertEqual(len(set(ariadne_cluster.louvain(undirected(edges)).values())), 1)

    def test_isolated_notes_each_get_their_own_cluster(self):
        graph = undirected([("a", "b")], isolated=["y", "z"])
        labels = ariadne_cluster.louvain(graph)
    ...
    def test_edgeless_vault_gives_every_note_its_own_cluster(self):
        ...
        self.assertEqual(len(set(ariadne_cluster.louvain(graph).values())), 3)

    def test_empty_vault(self):
        self.assertEqual(ariadne_cluster.louvain({}), {})

    def test_single_note(self):
        self.assertEqual(ariadne_cluster.louvain({"a": set()}), {"a": 0})
    ...
    def test_dangling_neighbour_outside_the_vault_is_ignored(self):
        ...
        labels = ariadne_cluster.louvain({"a": {"b", "gone"}, "b": {"a"}})
    ...
    def test_a_one_way_neighbour_map_is_treated_as_an_edge(self):
        ...
        self.assertEqual(
            grouping(ariadne_cluster.louvain({"a": {"b"}, "b": set()})), {frozenset("ab")}
        )
        self.assertEqual(ariadne_cluster.shape({"a": {"b"}, "b": set()}, {"a": 0, "b": 0})["edges"], 0)

    def test_beats_the_all_in_one_partition_it_starts_from(self):
        labels = ariadne_cluster.louvain(TWO_TRIANGLES)
        self.assertGreater(ariadne_cluster.modularity(TWO_TRIANGLES, labels), 0.3)
...
class DeterminismTests(unittest.TestCase):
    def test_repeated_runs_agree(self):
        first = ariadne_cluster.louvain(TWO_TRIANGLES)
        for _ in range(5):
            self.assertEqual(ariadne_cluster.louvain(TWO_TRIANGLES), first)

    def test_input_insertion_order_does_not_change_the_answer(self):
        forward = ariadne_cluster.louvain(TWO_TRIANGLES)
        reversed_input = {k: TWO_TRIANGLES[k] for k in reversed(sorted(TWO_TRIANGLES))}
        self.assertEqual(ariadne_cluster.louvain(reversed_input), forward)
    ...
    def test_renaming_nodes_without_reordering_them_preserves_the_partition(self):
        ...
        labels = ariadne_cluster.louvain(renamed)
        self.assertEqual(grouping(labels), {frozenset("ABC"), frozenset("DEF")})


class ShapeTests(unittest.TestCase):
    def setUp(self):
        self.labels = ariadne_cluster.louvain(TWO_TRIANGLES)
        self.summary = ariadne_cluster.shape(TWO_TRIANGLES, self.labels)
    ...
    def test_counts_isolated_notes_separately_from_components(self):
        graph = undirected([("a", "b")], isolated=["y", "z"])
        summary = ariadne_cluster.shape(graph, ariadne_cluster.louvain(graph))
    ...
    def test_describe_shape_names_every_number_it_was_given(self):
        line = ariadne_cluster.describe_shape(self.summary)
...
class DevVaultFixtureTests(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        paths = list(ariadne_common.iter_markdown_files(str(DEV_VAULT), []))
        entries = [
            {"path": p, "links": ariadne_common.extract_links(Path(p).read_text())} for p in paths
        ]
        index = ariadne_common.build_name_index(paths)
        cls.neighbors = ariadne_cluster.adjacency_from_links(entries, index)
        cls.labels = ariadne_cluster.louvain(cls.neighbors)
    ...
    def test_has_a_disconnected_three_note_component(self):
        sizes = [len(g) for g in ariadne_cluster.components(self.neighbors)]
        self.assertIn(3, sizes)

    def test_has_a_single_note_orphan_component(self):
        groups = ariadne_cluster.components(self.neighbors)
    ...
    def test_the_harbour_ring_is_the_only_cluster_with_no_entry_point(self):
        ...
        entries = ariadne_cluster.cluster_entry_points(self.neighbors, self.labels)
    ...
    def test_every_other_cluster_has_a_full_entry_point(self):
        ...
        for entry in ariadne_cluster.cluster_entry_points(self.neighbors, self.labels):
    ...
    def test_modularity_is_high_enough_to_be_worth_clustering(self):
        self.assertGreater(ariadne_cluster.modularity(self.neighbors, self.labels), 0.5)
